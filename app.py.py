...
try:
    ...
    def read_from_csv_file():
        ...
        if not csv_file_path.is_file(users_data.csv):
    ...
    with open(csv_file_path, "r") as file: 
        ...
        line_count = 0
        for row in reader:
            print(f'\tUser ({row["Id"]}) username is ({row["Username"]}), and has this email: ({row["Email"]}).')
            #print(row)
            line_count += 1
    ...
    """ def write_to_csv_file():
        first_write = not csv_file_path.is_file(users_data.csv)
    ...
    def do_admin_login():
        ...
        if session.get('locked', False):
            ...
            accounts = read_from_csv_file(users_data.csv)

            #create a loop to traverse through the users
            for user in accounts:
                #if theres a match check if the password is correct
            ...
            #if no match is found increase the failed attempts
    ...
    def admin_dashboard():
        ...
        if session.get('username') != 'roger':
            ...
            return home()
        #if it is 'roger' then render the admin dashboard page
        return render_template('admindashboard.html')
 """
    ...
    def addUser():
        ...
        if session.get('username') != 'roger':
            flash("Access denied. Only Uncle Roger can access this page.")
            return home()
        #if it is 'roger' then render adduser page (just exit if statement above and continue code)
        #create a hashing password function or hash and salt pw right in this function?
        pass #complete @Henry """
    ...
    def viewAccounts():
        ...
        if session.get('username') != 'roger':
            flash("Access denied. Only Uncle Roger can access this page.")
            return home()
        #if it is 'roger' then render accounts page (just exit if statement above and continue code)
